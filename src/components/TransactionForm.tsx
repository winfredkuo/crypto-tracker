...
export default function TransactionForm({ onAdd, onUpdate, currentExchangeRate, transactions, editData, templateData, onCancelEdit }: Props) {
  ...
  const handleFromAmountChange = (val: string) => {
    ...
    if (from > 0 && rate > 0) {
      let to = 0;
      // If ETH/SOL pair, rate is SOL/ETH (0.033)
      if ((fromAsset === 'ETH' && toAsset === 'SOL') || (fromAsset === 'SOL' && toAsset === 'ETH')) {
         // If From=ETH, To=SOL. To = From / Rate (1 / 0.033 = 30)
         if (fromAsset === 'ETH') to = from / rate;
         // If From=SOL, To=ETH. To = From * Rate (30 * 0.033 = 1)
         else to = from * rate;
      } else {
         to = from * rate;
      }
  ...
  const handleToAmountChange = (val: string) => {
    ...
    if (to > 0 && rate > 0) {
      let from = 0;
      if ((fromAsset === 'ETH' && toAsset === 'SOL') || (fromAsset === 'SOL' && toAsset === 'ETH')) {
         // If From=ETH, To=SOL. From = To * Rate (30 * 0.033 = 1)
         if (fromAsset === 'ETH') from = to * rate;
         // If From=SOL, To=ETH. From = To / Rate (1 / 0.033 = 30)
         else from = to / rate;
      } else {
         from = to / rate;
      }
  ...
  const handleAssetExchangeRateChange = (val: string) => {
    ...
    if (rate > 0) {
      if ((fromAsset === 'ETH' && toAsset === 'SOL') || (fromAsset === 'SOL' && toAsset === 'ETH')) {
         if (from > 0) {
            // Recalc To
            if (fromAsset === 'ETH') setToAmount((from / rate).toFixed(8).replace(/\.?0+$/, ''));
            else setToAmount((from * rate).toFixed(8).replace(/\.?0+$/, ''));
         } else if (to > 0) {
            // Recalc From
            if (fromAsset === 'ETH') setFromAmount((to * rate).toFixed(8).replace(/\.?0+$/, ''));
            else setFromAmount((to / rate).toFixed(8).replace(/\.?0+$/, ''));
         }
      } else {
         if (from > 0) {
  ...
  useEffect(() => {
    ...
    if (from > 0 && to > 0) {
      let rate = 0;
      if ((fromAsset === 'ETH' && toAsset === 'SOL') || (fromAsset === 'SOL' && toAsset === 'ETH')) {
        // Always calculate SOL/ETH (0.033)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'EXCHANGE') {
      ...
      // If ETH/SOL pair, we displayed SOL/ETH (0.033), but want to store ETH/SOL (30)
      if ((fromAsset === 'ETH' && toAsset === 'SOL') || (fromAsset === 'SOL' && toAsset === 'ETH')) {
          rate = 1 / rate;
      }
